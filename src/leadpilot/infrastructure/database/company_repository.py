...
from leadpilot.infrastructure.database.models import CompanyModel
...
class CompanyRepository:
    ...
    def list_all(self) -> list[Company]:
        with self._session_factory() as session:
            models = session.scalars(select(CompanyModel).order_by(CompanyModel.name))
    ...
    def search(self, query: str) -> list[Company]:
        ...
        with self._session_factory() as session:
            statement = (
                select(CompanyModel)
                .where(or_(*(func.lower(field).like(pattern) for field in fields)))
    ...
    def count(self) -> int:
        with self._session_factory() as session:
            return session.scalar(select(func.count(CompanyModel.id))) or 0
